...
from cellable.ai import EfficientSamVitS,SegmentAnythingModelVitH
import cellable.ai
import os
import queue
import argparse
...
def initializeAiModel(model_name):
    if model_name not in [model.name for model in cellable.ai.MODELS]:
        raise ValueError("Unsupported ai model: %s" % model_name)
    model = [model for model in cellable.ai.MODELS if model.name == model_name][0]
